...
plot.returns.validation<- function(variable){
  ...
  #Calculate K-S.-Distance and test statistic: 
  ks.stat_p1<- ks.test(unlist(d.valid[, variable, with=F]), unlist(d.valid.p1[, variable, with=F]))
  ks.stat_p2<- ks.test(unlist(d.valid[, variable, with=F]), unlist(d.valid.p2[, variable, with=F]))
  ks.stat_p3<- ks.test(unlist(d.valid[, variable, with=F]), unlist(d.valid.p3[, variable, with=F]))
  ...
  #Real set
  d5<- density(x=unlist(d.valid[, variable, with=F]))
  lines(d5, col="black")
  dev.off()
...
plot.returns.train<- function(variable){
  ...
  #Calculate K-S.-Distance and test statistic: 
  ks.stat_p1<- ks.test(unlist(d.train[, variable, with=F]), unlist(d.train.p1[, variable, with=F]))
  ks.stat_p2<- ks.test(unlist(d.train[, variable, with=F]), unlist(d.train.p2[, variable, with=F]))
  ks.stat_p3<- ks.test(unlist(d.train[, variable, with=F]), unlist(d.train.p3[, variable, with=F]))
  ...
  #Real set
  d5<- density(x=unlist(d.train[, variable, with=F]))
  lines(d5, col="black")
  dev.off()
  #return(p1)
}
