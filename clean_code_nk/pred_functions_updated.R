...
add_returns <- function(train, test, standard_value = 0.5) {
  ...
  train$return_per_articleID<- return_per_articleID(train)
  ...
  test <- merge(test,unique(train[,.(return_per_articleID),by=articleID]), by="articleID",all.x = TRUE)
  ...
  test[,return_per_articleID := replace(return_per_articleID,is.na(return_per_articleID),standard_value)]
...
# return per articleID week
return_per_articleID <- function(dt){
  dt[, new := mean(returnQuantity,na.rm=TRUE), by = articleID]
  dt[is.na(dt$new)]$new <- 0.5
  return(dt$new)
}

# return per articleID
return_per_articleID_week <- function(dt){
  dt[, new := mean(returnQuantity,na.rm=TRUE), by = .(articleID, week)]
  dt[is.na(dt$new)]$new <- 0.5
  return(dt$new)
}

# return per articleID month
return_per_articleID_month <- function(dt){
  dt[, new := mean(returnQuantity,na.rm=TRUE), by = .(articleID, month)]
  dt[is.na(dt$new)]$new <- 0.5
  return(dt$new)
}
